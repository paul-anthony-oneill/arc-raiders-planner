...
const ClickHandler: React.FC<{ onClick: (latlng: L.LatLng) => void }> = ({
  onClick,
}) => {
  useMapEvents({
    click(e) {
...
const MapEditor: React.FC<MapEditorProps> = ({ onExit }) => {
    ...
    if (map) {
      ...
      try {
        const areaRes = await fetch(
          `/api/maps/${encodeURIComponent(mapName)}/data`,
        );
        if (areaRes.ok) {
  ...
  const handleCalibrationClick = (latlng: L.LatLng) => {
    if (calPoints.length >= 4)
      return alert(
        "Calibration complete (4 points set). Click 'Calculate & Save'.",
      );
  ...
  const calculateAndSaveCalibration = async () => {
    ...
    console.log("Calculated 4-Point Calibration:", {
      scaleX,
      scaleY,
      offsetX,
      offsetY,
    ...
    if (!isFinite(scaleX) || !isFinite(scaleY))
      return alert("Invalid calculation.");
  ...
  const calculateCenter = (points: L.LatLng[]): { x: number; y: number } => {
    ...
    return {
      x: Math.round(totalLng / points.length),
      y: Math.round(totalLat / points.length),
    };
  };
    ...
    <div className="fixed inset-0 z-50 bg-retro-bg flex flex-col text-retro-sand font-mono">
      ...
      >
        ...
        <button
          ...
          style={{
            background: isCalibrating ? "#ff9800" : "#555",
            padding: "5px 15px",
            border: "none",
            cursor: "pointer",
          }}
        ...
        >
          {isCalibrating ? (
            ...
            >
              ...
              <p style={{ fontSize: "0.85em", lineHeight: "1.5" }}>
                <strong>X-Axis (Width):</strong>
                <br />
                1. Click Far <b>LEFT</b> point -{">"} Enter Coords
                <br />
                2. Click Far <b>RIGHT</b> point -{">"} Enter Coords
                <br />
                <br />
                <strong>Y-Axis (Height):</strong>
                <br />
                3. Click Far <b>TOP</b> point -{">"} Enter Coords
                <br />
          ...
          >
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
              {Array.from(new Set(gameMarkers.map((m) => m.category))).map(
                (cat) => (
                  <label
                    key={cat}
                    style={{
              ...
              {!isCalibrating &&
                gameMarkers
                  ...
                  .map((marker) => (
                    ...
                    >
                      <Popup>
                        <strong>{marker.name || marker.category}</strong>
                        <br />
                        <small>{marker.subcategory}</small>
                        <br />
                        <small style={{ color: "#666" }}>
                          API: {marker.lng.toFixed(0)}, {marker.lat.toFixed(0)}
                        </small>
                      </Popup>
                    </CircleMarker>
                  ))}
