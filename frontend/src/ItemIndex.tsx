...
const ItemIndex: React.FC<ItemIndexProps> = ({ onItemSelected }) => {
    ...
    const fetchItems = async (search: string = '') => {
        ...
        try {
            ...
            setError(`Failed to fetch items. Check console for details. (Error: ${err instanceof Error ? err.message : 'Unknown'})`);
        } finally {
            setLoading(false);
        }
    };
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p style={{ margin: 0, fontSize: '14px' }}>
                                    ...
                                    <strong>{item.lootType ? item.lootType.name : ' (Crafted/Enemy Drop)'}</strong>
                                </p>
                            </div>
                        </div>
                    ))
