 ...
 */
const EnemyIndex: React.FC<EnemyIndexProps> = ({ onEnemyTypeSelected, selectedEnemyTypes }) => {
    ...
    const fetchEnemyTypes = async () => {
        ...
        try {
            ...
            setError(`Failed to fetch enemy types. Check console for details. (Error: ${err instanceof Error ? err.message : 'Unknown'})`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="enemy-index-container" style={{ maxWidth: '800px', margin: '0 auto', fontFamily: 'sans-serif' }}>
            <h2>ARC Enemy Types ({enemyTypes.length} Available)</h2>
            <p style={{ fontSize: '14px', color: '#666', marginBottom: '20px' }}>
                Select enemy types to target. The planner will route towards spawns of these enemies near your loot areas.
            </p>
            ...
            <div className="enemy-list" style={{ maxHeight: '60vh', overflowY: 'auto', border: '1px solid #ccc', padding: '10px' }}>
                {enemyTypes.length > 0 ? (
                    enemyTypes.map((enemyType) => (
                        <div
                            ...
                            style={{
                                ...
                                alignItems: 'center',
                                cursor: 'pointer',
                                backgroundColor: isEnemyTypeSelected(enemyType) ? '#e3f2fd' : 'white',
                        ...
                        >
                            ...
                            <div>
                                <h4 style={{ margin: 0, fontSize: '18px' }}>{capitalize(enemyType)}</h4>
                            </div>
                        </div>
                    ))
                ) : (
